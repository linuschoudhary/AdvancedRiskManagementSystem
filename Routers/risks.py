...
@router.get("")
def show_all_risks(db:Session=Depends(get_db),current_user:schema.Users=Depends(access(role_based_access.level_2))):
...
@router.get("/id")
def show_risk_by_id(risk_id:int,db:Session=Depends(get_db),current_user:schema.Users=Depends(access(role_based_access.level_2))):
...
def add_new_risk(details: schema.Risks, db:Session=Depends(get_db),current_user:schema.Users=Depends(access(role_based_access.level_2))):
...
def update_risk(risk_id:int,details:schema.UpdateRisks,db:Session=Depends(get_db),current_user:schema.Users=Depends(access(role_based_access.level_2))):
...
def delete_risk(risk_id:int,db:Session=Depends(get_db),current_user:schema.Users=Depends(access(role_based_access.level_2))):
